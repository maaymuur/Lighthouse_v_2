...
const ZoneNumbers = () => {
    ...
    const handleTouchStart = (e: React.TouchEvent) => {
        touchStartX = e.touches[0].clientX; // сохраняем координату начала касания
    };

    // Обработчик окончания касания
    const handleTouchEnd = (e: React.TouchEvent) => {
        touchEndX = e.changedTouches[0].clientX; // сохраняем координату окончания касания

        // Если свайп был влево (touchEndX < touchStartX), переключаем на следующее изображение
        if (touchStartX - touchEndX > 50) {
            handleNextImage();
        }

        // Если свайп был вправо (touchEndX < touchStartX), переключаем на предыдущее изображение
    ...
    return (
        <div className="zone_numbers_block" id='numbers'>
            <div className='zone_numbers_main'>
                ...
                <div
                    className='zone_numbers_pictures'
                    onTouchStart={handleTouchStart} // Обрабатываем начало касания
                    onTouchEnd={handleTouchEnd} // Обрабатываем окончание касания
                >
